...
The relative number of injured and deceased people per event (see `r figure("PieHuman", display = "cite")`) show that tornadoes are responsible of most of the cases: `r p_fatalities[EVTYPE=="TORNADO",]$sum` people died due to tornadoes and  `r p_injuries[EVTYPE=="TORNADO",]$sum` were injured, which is a `r p_fatalities[EVTYPE=="TORNADO",]$sum *100 / sum(p_fatalities$sum)`% and a `r p_injuries[EVTYPE=="TORNADO",]$sum *100 / sum(p_injuries$sum)` of the total respectively).
...
The material costs of storm-related events are summarized in a relative comparison plot (see `r figure("PieStuff", display = "cite")`) and a bar plot with the total costs in US $ (see `r figure("BarMoney", display = "cite")`). `r figure("PieStuff", display = "cite")` shows that ice storms are the main responsible of crop damage and floods are the main responsible of material damages in private property. Finally, the results shown in `r figure("BarMoney", display = "cite")`, clearly indicate that ice storms are responsible of most of the costs.
